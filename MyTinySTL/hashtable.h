...
namespace mystl {
	...
	// __hashtable_iterator 的 operator++ 
	template <class Val, class Key, class HashFcn,
		class ExtractKey, class EqualKey, class Alloc>
	...
	// __hashtable_const_iterator 的 operator++ 
	template <class Val, class Key, class HashFcn,
		class ExtractKey, class EqualKey, class Alloc>
	...
	private:
		...
		typedef vector<node*, Alloc>	bucket_type;	//用 vector 作为链表的表现
	...
	public:
		...
		// 以下声明 friend 为了让迭代器能访问 hashtable 的私有成员数据和私有成员函数
	...
	pair<typename hashtable<Val, Key, HashFcn, ExtractKey, EqualKey, Alloc>::iterator, bool> 
		...
		auto tmp = __new_node(value);	//产生新节点
		tmp->next = first;
	...
	typename hashtable<Val, Key, HashFcn, ExtractKey, EqualKey, Alloc>::reference 
		...
		for (auto cur = first; cur; cur = cur->next)
			if (equal_(get_key_(cur->value), get_key_(value)))
				return cur->value;
		auto tmp = __new_node(value);	//新建节点
		tmp->next = first;
		buckets_[n] = tmp;	//让新节点成为链表的第一个节点
		++element_nums_;
		return tmp->value;
		...
		try {
			for (size_type i = 0; i < ht.buckets_.size(); ++i) {
				...
				if (cur) {	//如果某 bucket 存在链表
					auto copy = __new_node(cur->value);
					buckets_[i] = copy;
