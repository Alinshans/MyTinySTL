    ...
    while (x != root && x->parent->color == __rb_tree_red) {  // 当父节点为红
        if (x->parent == x->parent->parent->left) {           // 如果父节点是左子节点
            ...
            if (y && y->color == __rb_tree_red) {             // 如果伯父节点存在且为红
                ...
                if (x == x->parent->left) {                   // 如果 x 为父节点的左子节点
                    x = x->parent;
...
insert_unique(const value_type& value) {
    ...
    if (comp) {                                           // 离开循坏时 comp 为 true，在左边插入
        if (j == begin()) {
...
insert_unique(iterator position, const value_type& value) {
    if (position.node == header_->left) {        // 位于 begin 处
        if (size() && key_compare(KeyOfValue()(value), key(position.node))) { 
...
erase(iterator position) {
    auto y = reinterpret_cast<link_type>(
        __rb_tree_rebalance_for_erase(position.node, header_->parent, 
