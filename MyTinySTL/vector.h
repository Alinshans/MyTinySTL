...
public:
  ...
  vector(std::initializer_list<T> ilist) { __range_initialize(ilist.begin(), ilist.end()); }
...
template <class T, class Alloc>
inline vector<T, Alloc>::
vector(vector&& rhs) :start_(rhs.start_),
...
// 复制赋值操作符
template <class T, class Alloc>
inline vector<T, Alloc>& vector<T, Alloc>::
...
// 在 position 位置插入元素                                     
template <class T, class Alloc>
inline typename vector<T, Alloc>::iterator vector<T, Alloc>::
...
template <class InputIterator>
inline void vector<T, Alloc>::
__vector_initialize(InputIterator first, InputIterator last, __false_type)
{
...
template <class InputIterator>
inline void vector<T, Alloc>::
__range_initialize(InputIterator first, InputIterator last)
{
  auto n = last - first;
...
template <class InputIterator>
inline void vector<T, Alloc>::
__range_assign(InputIterator first, InputIterator last, input_iterator_tag)
{
  auto cur = begin();
...
template <class ForwardIterator>
void vector<T, Alloc>::
__range_assign(ForwardIterator first, ForwardIterator last, forward_iterator_tag)
{
  auto len = distance(first, last);
  if (static_cast<size_type>(len) > capacity())
  {    // 如果区间长度大于容器容量
    __destroy_and_deallocate();
    __range_initialize(first, last);
  }
...
__fill_insert(iterator position, size_type n, const value_type& value)
{
  ...
  if (static_cast<size_type>(end_of_storage_ - finish_) >= n)
  {
    ...
    const auto after_elems = finish_ - position;
    ...
    if (static_cast<size_type>(after_elems) > n)
    {
      ...
      mystl::fill_n(position, n, value_copy);
    }
    else
    {
      mystl::uninitialized_fill_n(finish_, n - after_elems, value_copy);
...
template <class InputIterator>
inline void vector<T, Alloc>::
__range_insert(iterator position, InputIterator first, InputIterator last,
               input_iterator_tag)
{
...
template <class ForwardIterator>
void vector<T, Alloc>::
__range_insert(iterator position, ForwardIterator first, ForwardIterator last,
