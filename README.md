...
=======
####简介
   这是一个小型的STL，为了练习`数据结构`、`模板编程`、`c++11`和`项目规范`等，而做的一个练习。这个小型的STL实现了绝大部分STL中原有的函数、容器，加入了一些c++标准库中的内容，部分作了修改，也有加入自己的一些部分。这个项目只作为**个人学习用途**，请勿用作其它用途。如发现错误，还请在`Issues`中指出。欢迎`Fork`和`Pull requests`改善、补充代码。<br>
